...
#     dev_labels = dev_labels.to(device)
    ...
    with torch.no_grad():
        for i in tqdm(range(0, n, batch_size), desc='Predicting'):
            ...
            scores_ab = forward_ab(parallel_model, tokenized_feature_dict_dev, device, batch_indices, lm_only=True,panphon_features=True ) 
            scores, scores_ba = forward_ab(parallel_model, tokenized_feature_dict_dev, device, batch_indices, panphon_features=True)
    ...
    for n in range(n_iters):
        ...
        for j in tqdm(range(0, len(train_indices), chunk_size), desc='Creating batches for tokenizaton'):
            ...
            for i in tqdm(range(0, len(chunk_train_indices), new_batch_size), desc='Training'):
                ...
                scores, all_embed = forward_ab(parallel_model, tokenized_feature_dict, device, mini_batch_indices,panphon_features=True )
                token_embed = forward_ab(parallel_model, tokenized_feature_dict, device, mini_batch_indices, lm_only=True,panphon_features=True )
            ...
            scorer_folder = working_folder + f'/scorer_asqa_pan/chk_{n}'
        ...
        scorer_folder = working_folder + '/scorer_asqa_pan/'
        if not os.path.exists(scorer_folder):
            os.makedirs(scorer_folder)
        #model_path = scorer_folder + '/linear.chkpt'
        #torch.save(parallel_model.module.linear.state_dict(), model_path)
