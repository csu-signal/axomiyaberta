...
from transformers import (
    ...
    XLMTokenizer, XLMForTokenClassification,
)
...
if __name__ == '__main__':
    ...
    albert_model = XLMForTokenClassification.from_pretrained(
        'xlm-mlm-en-2048', num_labels=6, id2label=id2label, label2id=label2id
    ).to(device)
    ...
    working_folder = '/s/carnap/b/nobackup/signal/m3x/axberta/task_finetune/xlm'   
    ...
    for epoch in range(num_train_epochs):
        ...
        scorer_folder = working_folder + f'/asner_xlm_tagger/chk_{epoch}'
        ...
        for set in eval_loaders:
            ...
            if set =='test':
                ...
                with open(working_folder + f'/asner_xlm_tagger/chk_{epoch}/{set}_predictions.csv', "w") as f:
                    wr = csv.writer(f)
                    wr.writerows(all_predictions)

                with open(working_folder + f'/asner_xlm_tagger/chk_{epoch}/{set}_labels.csv', "w") as f:
                    wr = csv.writer(f)
                    wr.writerows(all_labels)


        albert_model.save_pretrained(scorer_folder + '/xlm_ner')
        tokenizer.save_pretrained(scorer_folder + '/xlm_ner')
