...
from evaluate_metrics_ner_wiki import * #In this case for WikiNER, import metrics for this task specifically. 
...
#     wikiexamples_train = wikiexamples_train ]
    wikiexamples_test = ner.get_examples('as', 'as-test')
    print("train wiki NER",len(wikiexamples_train)), print("test", len(wikiexamples_test)), print(len(wikiexamples_dev))
...
#     asner_examples_test = asner.cleanify_tokens(asner_examples_test)
    #print(len(asner_examples_train)), print(len(asner_examples_test)), print(len(asner_examples_dev))
    
    #print("Loaded Assamese NER Train, Test and Dev datasets successfully")
    
    label_list = []
    for i, j in enumerate(wikiexamples_train):
...
#     print(label_list)
    ...
    max_panpad = find_panpad_maxlength(wikiexamples_train, wikiexamples_dev, wikiexamples_test)
...
#     ).to(device)
    ...
    train_dataloader = make_loader(features_train, 40, max_overall)
    eval_dataloader = make_loader(features_dev,20, max_overall)
    test_dataloader = make_loader(features_test, 20, max_overall)
    ...
    for epoch in range(num_train_epochs):
        ...
        for set in eval_loaders:
            ...
            for batch in loader:
                ...
                batch_results, accuracy = compute_metrics(predictions, labels,id2label, mode="WikiNER")
...
#         scorer_folder = working_folder + f'/wikiner_indic_tagger/chk_{epoch}'
...
#     tokenizer.save_pretrained(scorer_folder + '/indic_ner')
    ...
    val_df.to_csv(working_folder + '/val_results_wiki_pan_lr_2e_5_partial.csv', index=False)

    test_df = pd.DataFrame(test_results, columns=['epoch', 'precision', 'recall', 'f1', 'accuracy', 'loss'])
    test_df.to_csv(working_folder + '/test_results_wiki_panls_2e_5_partial.csv', index=False)
